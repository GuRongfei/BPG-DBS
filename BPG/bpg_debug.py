...
class BPG(ActorCriticRLModel):
            ...
            with self.graph.as_default():
                ...
                with tf.variable_scope("loss", reuse=False):
                    ...
                    if self.clip_range_vf_ph is None:
                        # No clipping
                        vpred_clipped = train_model.q_flat
                    else:
                        # Clip the different between old and new value
                        # NOTE: this depends on the reward scaling
                        vpred_clipped = self.old_vpred_ph + \
                            tf.clip_by_value(train_model.q_flat - self.old_vpred_ph,
                    ...
                    """no gradient:
                    loss = tf.abs(self.old_vpred_ph)"""
                    #loss = self.vf_loss

                    loss = self.pg_loss - self.entropy * self.ent_coef + self.vf_loss * self.vf_coef


                    #loss = tf.abs(self.grad_q_ph)
    ...
    def _train_step(self, learning_rate, cliprange, obs, returns, masks, actions, q_values, neglogpacs, grad_q, true_reward, update,
        ...
        """
        advs = returns - q_values
        advs = (advs - advs.mean()) / (advs.std() + 1e-8)
        td_map = {self.train_model.obs_ph: obs, self.action_ph: actions,
                  self.advs_ph: advs, self.rewards_ph: returns,
                  self.learning_rate_ph: learning_rate, self.clip_range_ph: cliprange,
                  self.old_neglog_pac_ph: neglogpacs, self.old_vpred_ph: q_values,
        ...
        if writer is not None:
            ...
            policy_loss, value_loss, policy_entropy, approxkl, clipfrac, _, new_q = self.sess.run(
                [self.pg_loss, self.vf_loss, self.entropy, self.approxkl, self.clipfrac, self._train, self.new_q], td_map)
        ...
        print('old_q: ', q_values[:4])
        print('new_q: ', new_q[:4])
        print("returns: ", returns[:4])
        print("true_reward: ", true_reward[:4])
    ...
    def learn(self, total_timesteps, callback=None, log_interval=1, tb_log_name="PPO2",
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, n_updates + 1):
                ...
                # Unpack
                obs, returns, masks, actions, q_values, neglogpacs, grad_q, states, ep_infos, true_reward = rollout
                self.returns_log.append(np.mean(returns))
                self.true_rewards_log.append(np.mean(true_reward))
                ...
                if states is None:  # nonrecurrent version
                    ...
                    for epoch_num in range(self.noptepochs):
                        ...
                        for start in range(0, self.n_batch, batch_size):
                            ...
                            slices = (arr[mbinds] for arr in (obs, returns, masks, actions, q_values, neglogpacs, grad_q, true_reward))
                    ...
                    for epoch_num in range(self.noptepochs):
                        ...
                        for start in range(0, self.n_envs, envs_per_batch):
                            ...
                            slices = (arr[mb_flat_inds] for arr in (obs, returns, masks, actions, q_values, neglogpacs, grad_q))
                ...
                if self.verbose >= 1 and (update % log_interval == 0 or update == 1):
                    explained_var = explained_variance(q_values, returns)
...
class Runner(AbstractEnvRunner):
    ...
    def _run(self):
        ...
        mb_obs, mb_rewards, mb_actions, mb_q_values, mb_dones, mb_neglogpacs, mb_grad_q = [], [], [], [], [], [], []
        ...
        for step in reversed(range(self.n_steps)):
            if step == self.n_steps - 1:
                ...
                nextvalues = mb_q_values[step + 1]
            delta = mb_rewards[step] + self.gamma * nextvalues * nextnonterminal - mb_q_values[step]
            mb_advs[step] = last_gae_lam = delta + self.gamma * self.lam * nextnonterminal * last_gae_lam
        mb_returns = mb_advs + mb_q_values

        mb_obs, mb_returns, mb_dones, mb_actions, mb_q_values, mb_neglogpacs, mb_grad_q, true_reward = \
            map(swap_and_flatten, (mb_obs, mb_returns, mb_dones, mb_actions, mb_q_values, mb_neglogpacs, mb_grad_q, true_reward))

        return mb_obs, mb_returns, mb_dones, mb_actions, mb_q_values, mb_neglogpacs, mb_grad_q, mb_states, ep_infos, true_reward
