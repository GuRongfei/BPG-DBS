...
from BPG.ppo_policy import ActorCriticPolicy
...
class PPO2(ActorCriticRLModel):
    ...
    def __init__(self, policy, env, gamma=0.99, n_steps=128, ent_coef=0.01, learning_rate=2.5e-4, vf_coef=0.5,
        ...
        self.episode_cnt = 0
        self.good_episode_cnt = 0
        self.episode_reward = 0
        self.episodes_reward = []
        self.actual_trainstep = None
    ...
    def learn(self, total_timesteps, callback=None, log_interval=1, tb_log_name="PPO2",
        ...
        self.actual_trainstep = total_timesteps
        ...
        with SetVerbosity(self.verbose), TensorboardWriter(self.graph, self.tensorboard_log, tb_log_name, new_tb_log) \
            ...
            for update in range(1, n_updates + 1):
                ...
                if not update % 100:
                    print("update: %s/%s" % (str(update), str(n_updates)))

                batch_size = self.n_batch // self.nminibatches
                t_start = time.time()
                frac = 1.0 - (update - 1.0) / n_updates
                lr_now = self.learning_rate(frac)*frac
                cliprange_now = self.cliprange(frac)*frac
                cliprange_vf_now = cliprange_vf(frac)*frac
                ...
                if self.terminate:
                    self.actual_trainstep = update*self.n_batch
                    break
...
class Runner(AbstractEnvRunner):
    ...
    def _run(self):
        ...
        for _ in range(self.n_steps):
            ...
            if self.model.episode_cnt == 0:
                self.model.episode_reward = 0
            self.model.episode_reward += np.mean(rewards)
            self.model.episode_cnt += 1
            if self.dones[0]:
                self.model.episode_reward /= self.model.episode_cnt
                self.model.episodes_reward.append(self.model.episode_reward)
                self.model.episode_cnt = 0

                if self.model.episode_reward > -0.05:
                    self.model.good_episode_cnt += 1
                elif self.model.episode_reward > -0.2:
                    self.model.good_episode_cnt += 0.2
                else:
                    self.model.good_episode_cnt = 0

                if self.model.good_episode_cnt >= 10:
                    self.model.terminate = True
                self.model.episode_reward = 0
